...
<%--
	1. 내장 객체 : 미리 생성된 객체 (JSP)
		1) request : 사용자가 보내준 데이터를 읽어올 때 사용한다.
		2) response : 요청처리 후 HTML/Cookie, 화면 이동
					    => 한개의 파일안에서 사용이 가능(지역변수)
						=>_jspService(HttpServletRequest request,
									 HttpServletResponse response)
						=> 매개변수 => 한번만 사용한다
		3) session : 사용자의 일부정보를 저장 => 프로그램 종료, 로그아웃 해제	
					 프로젝트에서 사용중인 모든 JSP에서 사용이 가능 (전역변수)
					 서버에 저장이 된다
					 예) 장바구니 ,댓글 , 예매(공통으로 id)
					 => 로그인 저장 (id, name...)
					 => 로그아웃시에 해제
					 *** Cookie는 클라이언트의 브라우저에 저장
					 *** Session는 서버에 저장

		4) pageContext : include(), forward()
						-------------------------여러개의 JSP가 request공유
		5) application : getRealPath() : 실제 JSP파일, 이미지파일 실제 경로명을 가지고 있다
		----------------------------Spring까지 사용이 된다
	...
	model1 => (JSP => JSP) : 사용하기 쉽다 , 간단한 Java사용만 하면된다
			 ...
			 =>분업이 불가능 (혼자 전체 사이트를 제작)
			 	HTML/CSS/JAVA/JavaScrpit
	  mv   => Model(자바) => VO, DAO, 요청처리 
	  		  View => 순수하게 태그형으로 출력
	  		  =>JSP에서 클래스 메소드를 호출해서 사용(완전하게 Java/HTML를 분리하지 못했다)
	model2 => (MVC) => 복잡하다 , 유지보수가 좋다
			  Model : 자바 ==> Back => 확장성, 재사용, 수정
			  View  : HTML/CSS ==> Front
			  Controller : 연결
			  
			  
		1. 로그인 처리 => 사용자 정보 (session에 저장)
	...
	-----------------------------------------------model 1 종료
	    ...
	    4. MV
	       JSP ==> Model ==> JSP
	    5. MVC
	    	   request 		request(요청처리)	   equest에 결과값을 담아서 전송
